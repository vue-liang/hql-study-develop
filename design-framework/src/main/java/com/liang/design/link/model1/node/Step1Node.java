...
@Slf4j
@Service
public class Step1Node extends AbstractLogicLink {
    @Override
    public ResultTest apply(RequestParameterTest requestParameter, LogicLinkFactory.DynamicContext dynamicContext) {
        dynamicContext.setId(String.valueOf(requestParameter.getRequestParameterId()));
        log.info("Step1Node 执行 requestParameter：{} dynamicContext：{}", requestParameter, dynamicContext);
